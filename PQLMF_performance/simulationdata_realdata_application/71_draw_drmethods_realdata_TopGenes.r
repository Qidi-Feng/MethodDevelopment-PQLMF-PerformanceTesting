...
source("~/scRNA_Method/Res_Summary/DRComparison-master/algorithms/ExtractW.R")
	tryCatch({
for (method_indx in seq(1,11)){
METHODS <- c("FA", "PCA", "ZINBWaVE", "pCMF", "DiffusionMap", "NMF", "PoissonNMF", "MDS", "LLE", "LTSA", "PQLMF")
#METHODS <- c("FA", "PCA", "NMF", "PoissonNMF", "MDS", "LLE", "LTSA", "PQLMF")
imethod <- METHODS[method_indx]
print(imethod)
num_pc <- 10
	file <- paste0(res_path,"res.",idata,".nPC",num_pc,".rpt",irpt,".",imethod,".top2000.rds")
	if(file.exists(file)){
	res <- readRDS(file)
...
save(data, file=paste0(figure_path, "res.",idata,".nPC",num_pc,".rpt",irpt,".allDRmethods.top2000.RData"))
library(ggplot2)
pngfile=paste0(figure_path,"res.",idata,".nPC",num_pc,".rpt",irpt,".allDRmethods.top2000.png")
