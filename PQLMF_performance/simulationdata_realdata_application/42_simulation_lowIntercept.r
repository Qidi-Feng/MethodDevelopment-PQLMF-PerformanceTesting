...
marker <- 1:marker_number
marker_counts <- simcount[which(rownames(simcount) %in% marker),]
#marker_counts <- SummarizedExperiment(marker_counts)
marker_counts <- SummarizedExperiment(assays=list(marker_counts=marker_counts))
altExp(sce, "marker_counts") <- marker_counts
saveRDS(sce, file=paste0("/net/mulan/home/qidif/scRNA_Method/Res_Summary/simulation_count/time",itime,".m_sigmag_sq",m_sigmag_sq,".nm_sigmag_sq",nm_sigmag_sq, ".sigmae_sq", sigmae_sq,".marker_number",marker_number,".prop",prop,".mu1.simu.rds"))
