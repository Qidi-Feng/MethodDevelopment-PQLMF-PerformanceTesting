...
##use Seurat to get the top 2000 variable genes.
library(Seurat)
Seurat_count <- CreateSeuratObject(counts = count, min.cells = 0, min.features = 0, project = "Seurat_count")
Seurat_count_vst <- FindVariableFeatures(object = Seurat_count, selection.method = "vst",nfeatures = 2000)
vst_features <- VariableFeatures(Seurat_count_vst)
count_vst <- count[which(rownames(count) %in% vst_features),]

##get marker counts by top1000 variable genes
Seurat_marker_vst <- FindVariableFeatures(object = Seurat_count, selection.method = "vst",nfeatures = 1000)
marker <- VariableFeatures(Seurat_marker_vst)
marker_counts <- count_vst[which(rownames(count_vst) %in% marker),]
##only use samples contain counts in marker genes.
marker_counts <- marker_counts[rowSums(marker_counts)>0,colSums(marker_counts)>0]
count_vst <- count_vst[,colnames(marker_counts)]
##format into sce
sce <- SingleCellExperiment(assays = list(counts = count_vst))
marker_counts <- SummarizedExperiment(assays=list(marker_counts=marker_counts))
altExp(sce, "marker_counts") <- marker_counts
saveRDS(sce, file=paste0("/net/mulan/home/qidif/scRNA_Method/Res_Summary/RealData_for_Clustering/data",idata,".top2000.marker1000.rds"))
}
